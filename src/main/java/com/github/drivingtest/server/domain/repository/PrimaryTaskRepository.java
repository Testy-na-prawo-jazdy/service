...
public interface PrimaryTaskRepository extends JpaRepository<PrimaryTask, Integer> {
    ...
    @Query(value = "select pt from PrimaryTask pt inner join pt.categories c where c.category =:#{#category} and pt.id not in (select lpt.primaryTask.id from LearnPrimaryTaskDetails lpt where lpt.user = :#{#user} and lpt.isCorrect = true) order by function('RAND')")
